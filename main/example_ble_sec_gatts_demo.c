...
static uint16_t custom_handle_table[CUSTOM_IDX_NB];
...
/// Custom Service
static const uint8_t custom_svc[16] =     {0x92,0xdd,0x7a,0x5e,0x52,0x85, 0xc8,0x90, 0x68,0x45, 0x60,0xa7, 0xd1,0x67,0x77,0x78};
static const uint8_t read_data_uuid[16] = {0x71,0xb1,0x10,0x3b,0x08,0x61, 0xe0,0x9b, 0x09,0x49, 0xdf,0xf4, 0x50,0xe7,0xbd,0x54};
static const uint8_t write_data_uuid[16] = {0x9a,0x15,0x22,0xe6,0x47,0xf2, 0xb9,0xab, 0x91,0x4e, 0xe5,0x42, 0x7f,0x8b,0x07,0x11};
...
static const uint8_t character_present_format[7] = {ESP_GATT_CPF_FORMAT_UINT32, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

/// custom Service - Read Data,Write Data
static uint8_t read_data[13] = {"initial value"};
static uint8_t write_data[13];

/// Custom service Database Description - Used to add attributes into the database
static const esp_gatts_attr_db_t custom_gatt_db[CUSTOM_IDX_NB] =
{
	...
	// Read Data Characteristic Declaration
    [CUSTOM_IDX_READ_CHAR]          =
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&character_declaration_uuid, ESP_GATT_PERM_READ,
    		CHAR_DECLARATION_SIZE,CHAR_DECLARATION_SIZE, (uint8_t *)&char_prop_read}},

    // Read Data Characteristic Value
    [CUSTOM_IDX_READ_DATA]             =
    {{ESP_GATT_RSP_BY_APP}, {ESP_UUID_LEN_128, (uint8_t *)&read_data_uuid, ESP_GATT_PERM_READ_ENCRYPTED,
      sizeof(read_data), sizeof(read_data), (uint8_t *)read_data}},

	[CUSTOM_DATA_IDX_CHARACTOR_PRESENT_FORMAT]        =
	{{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&character_present_format_uuid, ESP_GATT_PERM_READ,
	  sizeof(character_present_format),sizeof(character_present_format), (uint8_t *)character_present_format}},

    // Write Data Characteristic Declaration
    [CUSTOM_IDX_WRITE_CHAR]  =
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&character_declaration_uuid, ESP_GATT_PERM_READ,
      CHAR_DECLARATION_SIZE,CHAR_DECLARATION_SIZE, (uint8_t *)&char_prop_write}},

    // Write Data Characteristic Value
    [CUSTOM_IDX_WRITE_DATA]   =
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_128, (uint8_t *)&write_data_uuid, ESP_GATT_PERM_WRITE_ENCRYPTED,
      sizeof(write_data), sizeof(write_data),(uint8_t *)write_data}},
};
                                        ...
                                        esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    switch (event) {
        case ESP_GATTS_REG_EVT:
            esp_ble_gap_set_device_name(CUSTOM_DEVICE_NAME);
            //generate a resolvable random address
            esp_ble_gap_config_local_privacy(true);
            esp_ble_gatts_create_attr_tab(custom_gatt_db, gatts_if, CUSTOM_IDX_NB, CUSTOM_SVC_INST_ID);
            break;
        case ESP_GATTS_READ_EVT:
        	...
        	for(x = 0; x < sizeof(read_data); x++)
        	{
        		rsp.attr_value.value[x] = write_data[x];
        	}
            rsp.attr_value.len = sizeof(read_data);
            esp_ble_gatts_send_response(gatts_if, param->read.conn_id, param->read.trans_id, ESP_GATT_OK, &rsp);
            break;
        case ESP_GATTS_WRITE_EVT:
            ESP_LOGI(GATTS_TABLE_TAG, "ESP_GATTS_WRITE_EVT, write value:");
            esp_log_buffer_hex(GATTS_TABLE_TAG, param->write.value, param->write.len);
            for(x = 0; x < sizeof(write_data); x++)
            {
            	write_data[x] = *(param->write.value + x);
            }
            break;
        ...
        case ESP_GATTS_CREAT_ATTR_TAB_EVT: {
            ...
            if (param->create.status == ESP_GATT_OK){
                if(param->add_attr_tab.num_handle == CUSTOM_IDX_NB) {
                    memcpy(custom_handle_table, param->add_attr_tab.handles,
                    sizeof(custom_handle_table));
                    esp_ble_gatts_start_service(custom_handle_table[CUSTOM_IDX_SVC]);
                }else{
                    ESP_LOGE(GATTS_TABLE_TAG, "Create attribute table abnormally, num_handle (%d) doesn't equal to HRS_IDX_NB(%d)",
                         param->add_attr_tab.num_handle, CUSTOM_IDX_NB);
                }
            }else{
